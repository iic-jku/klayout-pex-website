...
The `KPEX` technology definition format uses [Google Protocol Buffers](https://protobuf.dev), so there is:

- formal schema files, defining the structure and data types involved
   - [`protos/tech.proto`]({{< var urls.repo.kpex >}}/blob/main/protos/kpex/tech/tech.proto): main schema / entry point, includes the others
   - [`protos/process_stack.proto`]({{< var urls.repo.kpex >}}/blob/main/protos/kpex/tech/process_stack.proto): describes details of the process stack, such as dielectrics and heights of layers
   - [`protos/process_parasitics.proto`]({{< var urls.repo.kpex >}}/blob/main/protos/kpex/tech/process_parasitics.proto): parasitic tables, used to parametrize the 2.5D engine
