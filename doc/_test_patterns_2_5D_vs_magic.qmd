...
- Version `Magic 8.3 rev 486` is used, but augmented with debug logging[^magic-with-debug-logging], which will be shown and discussed for each example.
   - NOTE: that revision still counted each sidewall edge against the full `defaultsidewall` value of the tech file, which `MAGIC 8.3.679` corrected (see @sec-magic-cap-types-sidewall-capacitance). Sidewall capacitances are given below under the corrected definition, which is half of what the `CapDebug` output of rev 486 reports.
...
Both edges together therefore couple `A` and `B` with $0.75\,fF$. Lines 18 and
21 of the `CapDebug` output below still report $0.75\,fF$ per edge, adding up
to $1.5\,fF$, because they were recorded with rev 486 --- the `MAGIC` column
above states that output under the definition of `MAGIC 8.3.679`.
...
## Test Pattern `r_wire_voltage_divider_li1` 
...
![](/figures/test_patterns/r_wire_voltage_divider_li1.png){fig-alt="Crossection through the via stack"}
...
GDS: <https://github.com/iic-jku/klayout-pex/blob/main/testdata/designs/sky130A/test_patterns/r_contact_2x2_minsize_mcon.gds.gz>
...
## Test Pattern `r_via_stack_1x1_minsize_poly_to_met5` 
...
On each metal layer, a pin is placed directly in middle of the via.
![](figures/test_patterns/r_via_stack_1x1_minsize_poly_to_met5.svg){fig-alt="Crossection through the via stack"}
...
## Test Pattern `nfet_li1` 

GDS: <https://github.com/iic-jku/klayout-pex/blob/main/testdata/designs/sky130A/test_patterns/nfet_li1.gds.gz>

![nfet example (2D layout view)](figures/test_patterns/r_nfet_li1.png){fig-alt="nfet example"}

![nfet example (3D view[^3D-view-created-by-kpex-fastercap])](figures/test_patterns/r_nfet_li1_3D_overview.png){#fig-resistance-extraction-nfet-overview}

@fig-resistance-extraction-nfet-overview gives an 3D overview over the 3 nets.
...
::: {.callout-warning}
To improve readability, the illustrations in @fig-resistance-extraction-nfet-net-D and @fig-resistance-extraction-nfet-net-G do not depict all nodes correctly in regard of the the z-axis orientation
...
![Resistance extraction of nfet example (only net `D`)"](figures/test_patterns/r_nfet_3D_D.svg){#fig-resistance-extraction-nfet-net-D}

@fig-resistance-extraction-nfet-net-D illustrates the parasitic resistance contributions for net `D` (all nets can be seen in @fig-resistance-extraction-nfet-overview). Net `G` is illustrated in @fig-resistance-extraction-nfet-net-G. Net `S` is not worth showing, as it is similar to net `D`.

![Resistance extraction of nfet example (only net `G`)"](figures/test_patterns/r_nfet_3D_G.svg){#fig-resistance-extraction-nfet-net-G}
